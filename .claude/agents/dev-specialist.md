...
- Review current components, hooks, types, queries, and mutations
- Understand the data flow between frontend and backend
- Check the `index.ts` for exported public API
- **Search for existing components** before creating new ones:
  - Check `shared/components/ui/` (shadcn primitives)
  - Check `shared/components/common/` (custom abstractions)
  - Check `features/*/components/` (feature-specific)
  - Use shadcn MCP tools to check available components
