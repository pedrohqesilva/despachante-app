...
import { FileText, Upload, Download, FileCheck, Home, Receipt, ScrollText, Map, AlertTriangle } from "lucide-react"
...
interface DocumentRowProps {
  ...
  onDelete: (id: Id<"propertyDocuments">, name: string) => void
}
...
const DOCUMENT_TYPE_CONFIG: Record<PropertyDocumentType, { icon: typeof FileText; label: string }> = {
  deed: { icon: FileCheck, label: "Escritura" },
  registration: { icon: Home, label: "Matrícula" },
  property_tax: { icon: Receipt, label: "IPTU" },
  lien_certificate: { icon: ScrollText, label: "Certidão de Ônus" },
  blueprint: { icon: Map, label: "Planta/Projeto" },
...
const DOCUMENT_TYPES: { value: PropertyDocumentType; label: string }[] = [
  ...
  { value: "lien_certificate", label: "Certidão de Ônus" },
  { value: "blueprint", label: "Planta/Projeto" },
  { value: "other", label: "Outro" },
]

function DocumentRow({ doc, onDelete }: DocumentRowProps) {
  const url = useQuery(propertyDocumentsApi.queries.getUrl, { storageId: doc.storageId })
  const config = DOCUMENT_TYPE_CONFIG[doc.type as PropertyDocumentType] || DOCUMENT_TYPE_CONFIG.other
...
export function DocumentsSection({ propertyId }: DocumentsSectionProps) {
  ...
  }, [pendingFile, generateUploadUrl, createDocument, propertyId, fileQueue])
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setPendingFile({ ...pendingFile, type: value as PropertyDocumentType })}
                  >
